...
- **Week 2 — checkpoint 2 (proxy worker + click-to-play).** Hardware encoder detection at startup (`h264_videotoolbox` on Mac, `h264_nvenc`/`h264_qsv` on Win, falls back to `libx264`); 720p H.264 proxy generated in a per-import tokio task, audio-stripped (`-an`), **explicit BT.709 color tags** (`-color_primaries bt709 -color_trc bt709 -colorspace bt709 -color_range tv`) so output doesn't get auto-tagged as BT.601 by hosts; `+faststart` for streaming; audio-only and ≤720p sources skip generation (`status: "source"`); `clip:proxy_state` events drive UI badges `pending → generating → ready` (or `failed`); cache at `$APPCACHE/proxies/<hash>.mp4`. Per-clip-id **event buffer** in the zustand store so race conditions (events fire before `addClip` lands on the React side) don't drop the first events. Click-to-play: `MediaTile` is now a button — click toggles `selectedClipId` in the store; preview pane renders `<video src={convertFileSrc(source_path)} controls />` with native controls; audio-only clips show a clear "no video stream" message instead of a broken `<video>`. Tile selection styles: amber border when selected, focus ring for keyboard nav. All four verifications green: typecheck ✓, lint ✓ (1 non-blocking warn only), `vite build` 1.36 s (1598 modules), `cargo build` 1.47 s warm. `pnpm dev` boots sramp-desktop binary successfully.

## Now
**Week 3 — checkpoint 3a** (cadence B): track + playhead. Drop a clip from the library onto a timeline; clip blocks render; click on a ruler to set the playhead; preview pane shows the proxy at the playhead frame. Two checkpoints in week 3 (cadence B again — founder liked it).

## Next
- **Week 3 — checkpoint 3b:** trim handles + custom transport bar (replaces native `<video controls>`); concatenated proxy playback through the timeline at the playhead.
- **Week 4** — ramp overlay + preset library v1; **design freeze**.
- **Weeks 5–13** — see `PLAN.md` §15.
...
- **ATTEMPT 1 [L1] — Asset protocol scope changes; thumbs + video loading still broken despite `["**"]`.** CAUSE — a race condition between Rust's spawned thumb/proxy workers firing events and `import_clip` returning the clip to React via IPC: when the listener fires before React has `addClip`-ed the clip, `patchClip` finds no matching id and silently no-ops. FIX (real fix, in the store): per-clip-id event buffer (`pendingEvents: Record<id, {thumb_path?, proxy?}>`); listener appends to buffer when id is unknown, `addClip`/`addClips` drains it on insert. Subsequent events then patch normally. Also normalized scope back to `["$APPCACHE/**", "$APPCACHE", "$APPDATA/**", "$HOME/**", "/Volumes/**", "/Users/**"]` for week 3 — discovered the asset-side wasn't the bug (a Tauri 2 glob sandbox test of `Pattern("/Users/**")` against the user's actual paths matched correctly), but the broad-open `["**"]` was diagnostic-only.
- **ATTEMPT 2 [L1] — Several small bumps along the way:** (a) edits on `commands/media.rs` failed because `oldString`/`newString` field ordering drifted; rewrote the file with `Write`. (b) Rust E0597 on `state.hw_encoder.lock()` inside a `tokio::spawn` closure (State-guard borrow-lifetime); hoisted the lock to before the spawn so the closure receives an owned `String`. (c) Several `eprintln!` "borrow of moved value" — `format_args!` consumes `id`; fixed by `id.clone()` for the inner struct. (d) Wrapper script syntax error `; do` instead of `; then` — fixed; lesson: `sh -n scripts/with-clean-target.sh` after every wrapper change.
